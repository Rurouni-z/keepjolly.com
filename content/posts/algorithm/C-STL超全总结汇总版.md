...
文章转载自[行码棋](https://wyqz.top/p/870124582.html)，并稍作修改

务必做到**读万卷书，行万里路**。

> 另外C版本一定要对（可能要加编译参数`-std=c++11`），C11即可，C++17或20更好。

>使DEV支持C++20 ： [https://blog.csdn.net/qq_50285142/article/details/122930647](https://blog.csdn.net/qq_50285142/article/details/122930647)
>'\n' vs  'std::endl': [link](https://www.geeksforgeeks.org/endl-vs-n-in-cpp/)
...
> 故局部区域**不可以**开大长度array，但是可以开大长度`vector`。
...
| `c.resize(n, v)`               | 改变数组大小为`n`,值为`v`，如果没有默认赋值为`0` |
...
// 初始化
stack<int> s ({1,2,3,4}) // or (std::deque<int>{1,2,3,4}) 默认container为deque
...
priority_queue<int> q(less<int>(), vector<int>{1,2,3});
```
...
| 代码                   | 含义                                                         |
| ---------------------- | ------------------------------------------------------------ |
| `mp.find(key)`         | 返回键为key的映射的迭代器 $O(logN)$ <br/>注意：find函数返回一个迭代器。当数据存在时，返回数据所在位置的迭代器，数据不存在时，返回$mp.end()$ |
...
int main() {
	map<int, int> m{{1, 2}, {2, 2}, {1, 2}, {8, 2}, {6, 2}};//有序，重复key删除
...
方式三：初始化时使用[匿名函数](https://www.cnblogs.com/pzhfei/archive/2013/01/14/lambda_expression.html)定义比较规则

```cpp
#include <functional>
set<int, std::function<bool(int, int)>> s([&](int i, int j){
    return i > j; // 从大到小
});
for(int i = 1; i <= 10; i++)
    s.insert(i);
...
## 总结
**[Containers library](https://en.cppreference.com/w/cpp/container "cpp/container")**

| 容器 | 迭代器类型 | Container | 有序？  | 时间复杂度 | 底层 | 备注 |
|--| --| -- | -- | -- |  -- | -- |
|[array](https://en.cppreference.com/w/cpp/container/array "cpp/container/array") | 随机访问(支持iter+2) | sequence |  | 随机[读改]([operator[]](https://en.cppreference.com/w/cpp/container/array/operator_at "cpp/container/array/operator at")) $O(1)$  | 静态数组 | array<int, 5> ay; 不能扩容  |
| [vector](https://en.cppreference.com/w/cpp/container/vector) | 随机访问 | sequence |  | [随机读改](https://en.cppreference.com/w/cpp/container/vector/operator_at)、[尾插](https://en.cppreference.com/w/cpp/container/vector/emplace_back)/[删](https://en.cppreference.com/w/cpp/container/vector/pop_back) $O(1)$<br>[插](https://en.cppreference.com/w/cpp/container/vector/insert)/[删](https://en.cppreference.com/w/cpp/container/vector/erase)$O(n)$  | 动态数组 | 是push_back()   |
| [deque](https://en.cppreference.com/w/cpp/container/deque) | 随机访问 | sequence  |  | [随机读改](https://en.cppreference.com/w/cpp/container/deque/operator_at)、[头插](https://en.cppreference.com/w/cpp/container/deque/push_front)/[删](https://en.cppreference.com/w/cpp/container/deque/pop_front)、[尾插](https://en.cppreference.com/w/cpp/container/deque/push_back)/[删](https://en.cppreference.com/w/cpp/container/deque/pop_back) $O(1)$<br>[插](https://en.cppreference.com/w/cpp/container/deque/insert)/[删](https://en.cppreference.com/w/cpp/container/deque/erase)$O(n)$ | 双端队列(数组) | 是push_back/front() |
| [list](https://en.cppreference.com/w/cpp/container/list "cpp/container/list") | 双向访问 | sequence  |  | [插](https://en.cppreference.com/w/cpp/container/list/insert)/[删](https://en.cppreference.com/w/cpp/container/list/erase)$O(1)$ | 双向链表([deque区别](ttps://stackoverflow.com/questions/1436020/whats-the-difference-between-deque-and-list-stl-containers)) | 是push_back/front() |
| [map](https://en.cppreference.com/w/cpp/container/map "cpp/container/map") | 双向访问(支持it--)  | associative | 有 | [增](https://en.cppreference.com/w/cpp/container/map/insert)/[删](https://en.cppreference.com/w/cpp/container/map/erase)/[查](https://en.cppreference.com/w/cpp/container/map/operator_at)$O(log_n)$(但是多态导致不同复杂度) | 红黑树 | 只有insert() |
| [unordered_map](https://en.cppreference.com/w/cpp/container/unordered_map "cpp/container/unordered map") | 双向访问  | unordered associative | | [增](https://en.cppreference.com/w/cpp/container/unordered_map/insert)/[删](https://en.cppreference.com/w/cpp/container/unordered_map/erase)/[查](https://en.cppreference.com/w/cpp/container/unordered_map/operator_at)$O(1)$，最差$O(n)$ | 哈希表 |  只有insert() |
|[set](https://en.cppreference.com/w/cpp/container/set "cpp/container/set") | 双向访问 | associative | 有 | 增删改$O(log_n)$ | 红黑树 | 只有insert() |
| [unordered_set](https://en.cppreference.com/w/cpp/container/unordered_set "cpp/container/unordered set") | 双向访问 | unordered associative  |   | 增删查$O(1)$，最差$O(n)$ | 哈希表 | 只有insert() |
| [stack](https://en.cppreference.com/w/cpp/container/stack "cpp/container/stack") | 不支持 | Container adaptor  |  |  顶删/增 $O(1)$（FIFO） | deque/list/vector | 不支持列表初始化，只有push() |
| [queue](https://en.cppreference.com/w/cpp/container/queue "cpp/container/queue") | 不支持 | Container adaptor  |  | 尾删/头插 $O(1)$（FILO） | deque/list | 不支持列表初始化，只有push() |
| [priority_queue](https://en.cppreference.com/w/cpp/container/priority_queue "cpp/container/priority queue") | 不支持 | Container adaptor | 最大/小在顶端 | 插入、删除 $O(log2n)$ | 堆 | 不支持列表初始化，只有push() |
...
>
> 在一些题目中，读入的**数据量很大**，往往超过了1e5（10^5）的数据量,而`cin`和`cout`的读入输出的速度**很慢**（是因为`cin`和`cout`为了兼容C语言的读入输出在性能上做了妥协），远不如`scanf`和`printf`的速度，具体原因可以搜索[相关的博客](https://blog.csdn.net/qq_67473550/article/details/130890891)进行了解。
>
...
指向底层元素存储的**指针**。对于非空容器，返回的指针与首元素地址比较相等。
```cpp
...
## iota
[C++ 标准库中的函数 iota 是什么的缩写？ - 知乎](https://www.zhihu.com/question/277961910)
```
iota(beg, end)
```
...
`nth`为一个迭代器，指向序列中的一个元素。**下标**第n小的值恰好在`nth`位置上。
